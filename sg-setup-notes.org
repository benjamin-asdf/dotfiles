...
* screencasts
- walkthrough use my script:

[[file:scripts/.local/bin/scripts/video-selected]]
...
- it remembers the pid of the process it starts
- it kills the current video process, so you can call the same script
  to stop recording
- else it asks you to select something
...
- if you know how to do it, you can add a marker to your wm modeline,
  as long as /tmp/recordingpid exists, you are recording
 [[file:stumpwm/.stumpwm.d/init.lisp]]
...
** 1 script to upload to s3
   ...
   I want things slow and deliberate so I only upload with a second
   keybind, instead of screen capturing and uploading.

** screen capture

[[file:scripts/.local/bin/scripts/video-selected][file:scripts/.local/bin/scripts/video-selected]]

depends on a package slop-git
