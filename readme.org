...
shared packages live alongside machine-specific overlays.

** Install

#+begin_src sh
cd ~/dotfiles
stow bash mememacs qutebrowser ranger scripts \
     flameshot dunst mpv gtk dmenu \
     clojure dot-clojure bb bbin .clj-kondo .lsp calva \
     git-pack
#+end_src

If a target file already exists (e.g. distro-provided ~/.bash_profile),
move it aside first: `mv ~/.bash_profile ~/.bash_profile.pre-stow.bak`.
...
| gaia-s, gaia-s-stumpwm | 2024-10 | machine retired |
